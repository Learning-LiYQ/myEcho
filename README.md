...
Echo 是一套前后端不分离的开源社区系统，基于目前主流 Java Web 技术栈（SpringBoot + MyBatis + MySQL + Redis +  Spring Security + ...）。包含帖子、评论、私信、系统通知、点赞、关注、搜索、用户设置、数据统计等模块。
...
-  **注册**
  ...
  - 对用户输入信息的合法性进行检验，并使用随机盐值 + md5 算法对密码加密
  - 向用户邮箱发送激活邮件，用户点击链接后将其激活状态字段置为 1。
...
-  **优化网站性能**
  ...
  - 引入本地缓存 Caffeine，缓存热帖列表和帖子的总数，避免缓存雪崩。